...
def encoding(payload,audio_file,LSB_bit,isPath):
...
def decoding(LSB_bit):
    # Opens the embedded audio
    embedded_audio = wave.open("song_encoded.wav", mode='rb')
    ...
    if decodedData[-1:] == '$':
        ...
        with open("decoded_payload" + fileExt, "wb") as outFile:
